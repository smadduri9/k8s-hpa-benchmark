...
- Metrics cells are `TARGET_UNAVAILABLE` (not `MISSING`) when the row is unavailable: `ready_replicas == 0`, or (fixed arm) `ready_replicas` below declared capacity, or (available row) rate metrics absent while gauge exists. Coverage (`METRICS_COLUMN_COVERAGE`) is over available rows only; rate columns exclude burst-onset rows and rows where the cell is `TARGET_UNAVAILABLE`. Report `TARGET_AVAILABILITY rows_available/rows_total`. Never lower `MIN_COLUMN_COVERAGE_RATIO`.
