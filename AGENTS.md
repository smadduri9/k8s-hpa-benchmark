...
- Locust invocation is exactly: `--headless --csv <base> --csv-full-history --exit-code-on-error 0`. Never add `--processes` (crashes with `--csv-full-history`, locust#2908). Never pass `--users` or `--spawn-rate` on measured shapes (`LoadTestShape` overrides them; they are inert). Measured-arm success is artifact-based (stats CSV + shape completion), not Locust exit code. Warm-up runs (`locustfile_warmup.py`) have no `LoadTestShape` by design — they hold a constant minimum load below the HPA target, not a curve — and validate on stats CSV only (non-zero Aggregated request count); shape completion is not required there.
