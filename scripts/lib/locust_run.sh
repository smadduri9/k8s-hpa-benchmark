...
locust_start_bounded_users() {
  # Warm-up only. Locustfile must not define LoadTestShape; --users is otherwise inert.
  local locust_file="$1"
  local host="$2"
  local run_time="$3"
  local csv_base="$4"
  ...
  harness_echo "${harness_log}" "WARMUP_LOCUST users=${users} spawn_rate=${spawn_rate} run_time=${run_time}"
  harness_echo "${harness_log}" \
    "LOCUST_WALL_CLOCK_SEC=${wall_secs} run_time_sec=${run_secs} margin_sec=${LOCUST_WALL_MARGIN_SEC}"
